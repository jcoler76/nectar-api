import { Refresh as RefreshIcon, SmartToy as AIIcon } from '@mui/icons-material';
    ...
    <Box>
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 2 }}>
        ...
        {endpoint.metadata?.affectedTables?.map(table => (
          <Chip key={table} label={`Table: ${table}`} size="small" variant="outlined" />
        ))}
        {endpoint.metadata?.errorHandling?.hasErrorHandling && (
          <Chip label="Has Error Handling" size="small" color="warning" variant="outlined" />
        )}
        <Button
          size="small"
      ...
      <ResponseExamples samples={endpoint.responseExample} metadata={endpoint.metadata} />
    </Box>
  );
};
