...
class AnalyticsService {
     ...
     */
    static async getRevenueDashboard(period = 'MONTHLY') {
        const [revenueMetrics, subscriptionAnalytics, churnRate, upcomingRenewals, recentBillingEvents] = await Promise.all([
            this.getRevenueMetrics(period),
            stripeService_1.StripeService.getSubscriptionAnalytics(),
            stripeService_1.StripeService.calculateChurnRate(period),
            stripeService_1.StripeService.getUpcomingRenewals(30),
     ...
     */
    static async getRevenueMetrics(period, limit = 12) {
    ...
    static async calculateMRR() {
        const activeSubscriptions = await database_1.prisma.subscription.findMany({
            where: {
     ...
     */
    static async getSubscriptionGrowth(period) {
        const metrics = await database_1.prisma.revenueMetric.findMany({
            where: { period },
            orderBy: { date: 'asc' },
            take: 12,
            select: {
    ...
    static async getChurnAnalysis() {
        ...
        const [churnedByPlan, churnReasons, churnTrends] = await Promise.all([
            ...
            database_1.prisma.billingEvent.groupBy({
                by: ['description'],
                where: {
            ...
            database_1.prisma.revenueMetric.findMany({
                where: {
                    period: 'MONTHLY',
    ...
    static async getPlanDistribution() {
        const planCounts = await database_1.prisma.subscription.groupBy({
            by: ['plan'],
            where: {
                status: { in: ['ACTIVE', 'TRIALING'] }
            },
    ...
    static async getRecentBillingActivity(limit = 20) {
        return database_1.prisma.billingEvent.findMany({
            include: {
                organization: {
     ...
     */
    static calculateGrowthMetrics(revenueMetrics) {
     ...
     */
    static async calculateRevenueProjections(revenueMetrics) {
     ...
     */
    static calculateAverageGrowthRate(revenues) {
    ...
    static async getCustomerLifetimeValue() {
        const [avgMonthlyRevenue, avgChurnRate, avgCustomerLifespan] = await Promise.all([
            this.calculateARPU(),
            stripeService_1.StripeService.calculateChurnRate('MONTHLY'),
     ...
     */
    static async calculateAverageCustomerLifespan() {
        const canceledSubscriptions = await database_1.prisma.subscription.findMany({
            where: {
                status: 'CANCELED',
                canceledAt: { not: null }
            },
    ...
    static async generateExecutiveSummary() {
        const [mrr, arr, churnRate, subscriptionAnalytics, clv, upcomingRenewals] = await Promise.all([
            this.calculateMRR(),
            this.calculateARR(),
            stripeService_1.StripeService.calculateChurnRate('MONTHLY'),
            stripeService_1.StripeService.getSubscriptionAnalytics(),
