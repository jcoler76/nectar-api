...
class BillingController {
     ...
     */
    static async getBillingMetrics(req, res) {
        try {
            const now = new Date();
            ...
            const [successfulPayments, failedPayments, refunds, last7DaysEvents, pendingInvoices, allPaymentEvents] = await Promise.all([
            ...
            const successfulPaymentAttempts = allPaymentEvents.filter(event => ['invoice_paid', 'payment_succeeded'].includes(event.eventType)).length;
            ...
            const failedPaymentsList = failedPayments.map(payment => ({
                ...
                reason: payment.metadata?.failure_reason || 'Unknown reason',
            ...
            successfulPayments.forEach(payment => {
                const method = payment.metadata?.payment_method_type || 'card';
            ...
            const geoRevenue = await database_1.prisma.organization.groupBy({
                by: ['country'],
                where: {
                    country: { not: null },
                    subscription: {
