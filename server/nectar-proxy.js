...
async function authenticateUser(username, password) {
  ...
  const apiKey = process.env.NECTAR_API_KEY || process.env.REACT_APP_API_KEY;

  if (!apiKey) {
    return Promise.reject(
      new Error(
        'API key not found in environment variables. Please set NECTAR_API_KEY in .env file.'
      )
    );
  }
  ...
  const apiUrl = new URL(
    process.env.NECTAR_CONNECT_API_URL ||
      'http://localhost:3001/api/v2/mm/_proc/uspUserAuthenticationGetNectarConnect'
  );
    ...
    const req = client.request(options, res => {
      ...
      res.on('end', () => {
        ...
        try {
          ...
          if (
            responseData.ErrorMessage === 'Access denied' ||
            (responseData.value &&
              responseData.value[0] &&
              responseData.value[0].ErrorMessage === 'Access denied')
          ) {
