    ...
    for (const db of req.body.databases) {
      ...
      try {
        ...
        const transformedObjects = [
          // Procedures first
          ...objects
            .filter(obj => obj.object_category === 'PROCEDURE')
            .map(obj => ({
              path: `/proc/${obj.name}`,
            })),
          // Then views
          ...objects
            .filter(obj => obj.object_category === 'VIEW')
            .map(obj => ({
              path: `/view/${obj.name}`,
            })),
          // Then tables
          ...objects
            .filter(obj => obj.object_category === 'TABLE')
            .map(obj => ({
              path: `/table/${obj.name}`,
            })),
        ];
        ...
        logger.error(`Failed to fetch schema for service ${dbName}:`, { error: error.message });
      }
