...
router.post('/session', express.json(), async (req, res) => {
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      subscription_data: trialDays ? { trial_period_days: Math.min(Math.max(trialDays, 0), 30) } : undefined,
          ...
          if (subId) {
            ...
            if (sub) {
              const data = {
                ...
                trialEndsAt: stripeSub.trial_end ? new Date(stripeSub.trial_end * 1000) : sub.trialEndsAt,
              }
          ...
          if (stripeSubId) {
            ...
            if (sub) {
              ...
              const data = {
                ...
                paidAt: event.type === 'invoice.paid' ? new Date(inv.status_transitions?.paid_at * 1000 || Date.now()) : null,
              }
...
router.get('/session/:id', async (req, res) => {
  try {
    ...
    if (!secret) return res.status(501).json({ error: 'Stripe not configured' })
    let Stripe
    try { Stripe = require('stripe') } catch { return res.status(500).json({ error: 'Stripe SDK not installed on server' }) }
    const stripe = new Stripe(secret, { apiVersion: '2024-06-20' })
    ...
    res.json({
      ...
      subscription_id: session.subscription ? String(session.subscription.id || session.subscription) : null,
...
router.post('/portal', authMiddleware, express.json(), async (req, res) => {
  try {
    ...
    const returnUrl = process.env.BILLING_PORTAL_RETURN_URL || (process.env.PUBLIC_URL ? `${process.env.PUBLIC_URL}/billing` : 'http://localhost:3000/billing')
