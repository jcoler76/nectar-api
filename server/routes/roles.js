/**
 * Roles Routes - GraphQL Integration with Enhanced Security
 * SECURITY: All endpoints protected with comprehensive authorization middleware
 * RLS: Uses tenant-aware Prisma client per CLAUDE.md requirements
 */
...
// SECURITY: Import role-specific authorization middleware
const {
...
// SECURITY: Import comprehensive input validation middleware
const {
  ...
  sanitizeRoleInputs,
} = require('../middleware/roleInputValidation');
// SECURITY: Import CSP headers for iframe protection
const { roleManagementCSP, handleCSPReport } = require('../middleware/cspHeaders');

// SECURITY: Apply CSP headers to all role routes for iframe protection
router.use(roleManagementCSP());

// SECURITY: Get all roles - protected with read permissions
router.get('/', ...createRoleAuthStack('read'), createListHandler(ROLE_QUERIES.GET_ALL, 'roles'));

// SECURITY: Get single role by ID - protected with read permissions and role access verification
router.get(
  '/:id',
  ...createRoleAuthStack('read', { requireRoleAccess: true }),
  createGetHandler(ROLE_QUERIES.GET_BY_ID, 'role')
);

// SECURITY: Create new role - comprehensive security stack
router.post(
  '/',
...
// SECURITY: Update role - protected with update permissions, role access, and comprehensive validation
router.put(
  '/:id',
  ...createRoleAuthStack('update', { requireRoleAccess: true, requireOwnership: false }),
  ...validateRoleUpdate(),
  sanitizeRoleInputs(),
  async (req, res) => {
    const { id } = req.params;
...
// SECURITY: Delete role - protected with delete permissions and role access
router.delete(
  '/:id',
  ...createRoleAuthStack('delete', { requireRoleAccess: true, requireOwnership: false }),
  createDeleteHandler(ROLE_QUERIES.DELETE, 'deleteRole')
);

// SECURITY: Add permission to role - protected with update permissions
router.post(
  '/:id/permissions',
  ...createRoleAuthStack('update', { requireRoleAccess: true, requireOwnership: false }),
  async (req, res) => {
...
// SECURITY: Remove permission from role - protected with update permissions
router.delete(
  '/:id/permissions',
  ...createRoleAuthStack('update', { requireRoleAccess: true, requireOwnership: false }),
  async (req, res) => {
...
// SECURITY: Test role permissions - protected with read permissions
router.post(
  '/:id/test',
  ...createRoleAuthStack('read', { requireRoleAccess: true }),
  async (req, res) => {
...
// SECURITY: Refresh role schemas - protected with update permissions
router.post(
  '/:id/refresh-schemas',
  ...createRoleAuthStack('update', { requireRoleAccess: true }),
  async (req, res) => {
    const { id } = req.params;
    const context = createGraphQLContext(req);
...
// SECURITY: Get service schema for role creation - CRITICAL SECURITY ENDPOINT
// This endpoint was previously exposing database schemas without proper authorization
router.get(
  '/service/:serviceId/schema',
  ...createRoleAuthStack('read', { requireServiceAccess: true }),
  ...validateServiceSchema(),
  async (req, res) => {
    try {
    ...
    `;
      ...
      if (!objects || objects.length === 0) {
        ...
        try {
          // SECURITY: Use proper tenant-aware Prisma client per CLAUDE.md requirements
          const serviceWithConnection = await prismaService.withTenantContext(
          ...
          if (!serviceWithConnection?.connection) {
            ...
            // SECURITY: Update the service using proper tenant-aware client
      ...
      if (!Array.isArray(objects)) {
        logger.error('Objects is not an array after processing:', {
          objects,
          type: typeof objects,
        });
        objects = [];
...
// SECURITY: Get roles for a specific service - protected with read permissions
router.get(
  '/service/:serviceId',
  ...createRoleAuthStack('read', { requireServiceAccess: true }),
  async (req, res) => {
...
// SECURITY: Batch operations for roles - DANGEROUS ENDPOINT - requires high-level permissions
router.post(
  '/batch',
  ...createRoleAuthStack('manage'), // Higher permission level for batch operations
  ...validateBatchOperation(),
  async (req, res) => {
...
router.post('/csp-report', handleCSPReport());
