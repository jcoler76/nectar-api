...
const { verifyApplicationAccess } = require('../middleware/resourceAuthorization');
...
router.get('/', async (req, res) => {
  try {
    ...
    // Add apiKey virtual field for backward compatibility
    const appsWithApiKey = applications.map(app => {
      const appObj = app.toObject({ virtuals: true });
      return appObj;
    });

    res.json(appsWithApiKey);
  } catch (error) {
...
router.post('/', validate(validationRules.application.create), async (req, res) => {
  try {
    const { name, description, defaultRole, isActive, apiKey: customApiKey } = req.body;

    // Use custom API key if provided, otherwise generate new one
    let apiKey;
    if (customApiKey && customApiKey.trim()) {
      ...
      const isUnique = await Application.isApiKeyUnique(apiKey);
      if (!isUnique) {
        return res.status(400).json({
          success: false,
